...
- (void)viewWillAppear:(BOOL)animated {
	...
	// set currency symbol
	tempString = [[NSString alloc] initWithFormat:@"%@",[[NSLocale currentLocale] objectForKey:NSLocaleCurrencySymbol]];
...
- (void)setAllButtons {
	...
	for(int ind=0;ind<kNumberOfKeyPadButtons;ind++){
		...
		tempRect = CGRectMake(kButtonCoordinate[buttonIndex][0],kButtonCoordinate[buttonIndex][1],kButtonCoordinate[buttonIndex][2],kButtonCoordinate[buttonIndex][3]);
		keyPadButtons[ind].frame = tempRect;
		[keyPadButtons[ind] setTitle:@"" forState:UIControlStateNormal];
		[keyPadButtons[ind] addTarget:self action:@selector(keyPadButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
		buttonIndex++;
	}	
	...
	for(int ind=0;ind<kTipPercentAdjustButtons;ind++){
		...
		if(ind==0)
		{
			[tipPercentAdjustButtons[ind] setImage:[UIImage imageNamed:kTipPercentAdjustButtonIncreaseNormal] forState:UIControlStateNormal];
			[tipPercentAdjustButtons[ind] setImage:[UIImage imageNamed:kTipPercentAdjustButtonIncreaseHighlighted] forState:UIControlStateHighlighted];		
		}
		else {
			[tipPercentAdjustButtons[ind] setImage:[UIImage imageNamed:kTipPercentAdjustButtonDecreaseNormal] forState:UIControlStateNormal];
			[tipPercentAdjustButtons[ind] setImage:[UIImage imageNamed:kTipPercentAdjustButtonDecreaseHighlighted] forState:UIControlStateHighlighted];
		}
	...
	// add action
	[tipPercentAdjustButtons[0] addTarget:self action:@selector(increaseTipPercent:) forControlEvents:UIControlEventTouchUpInside];
	[tipPercentAdjustButtons[1] addTarget:self action:@selector(decreaseTipPercent:) forControlEvents:UIControlEventTouchUpInside];
	...
	for(int ind=0;ind<kTipPercentAdjustButtons;ind++){
		...
		if(ind==0){
			[numberOfPeopleAdjustButtons[ind] setImage:[UIImage imageNamed:kNumberOfPeopleAdjustButtonIncreaseNormal] forState:UIControlStateNormal];
			[numberOfPeopleAdjustButtons[ind] setImage:[UIImage imageNamed:kNumberOfPeopleAdjustButtonIncreaseHighlighted] forState:UIControlStateHighlighted];		
		}
		else {
			[numberOfPeopleAdjustButtons[ind] setImage:[UIImage imageNamed:kNumberOfPeopleAdjustButtonDecreaseNormal] forState:UIControlStateNormal];
			[numberOfPeopleAdjustButtons[ind] setImage:[UIImage imageNamed:kNumberOfPeopleAdjustButtonDecreaseHighlighted] forState:UIControlStateHighlighted];
		}
	...
	// add action
	[numberOfPeopleAdjustButtons[0] addTarget:self action:@selector(increaseNumberOfPeople:) forControlEvents:UIControlEventTouchUpInside];
	[numberOfPeopleAdjustButtons[1] addTarget:self action:@selector(decreaseNumberOfPeople:) forControlEvents:UIControlEventTouchUpInside];
	
	//Tip Method segmented control
	tipMethodControlSwitch= [[UISegmentedControl alloc] initWithItems:nil];
	tipMethodControlSwitch.frame = CGRectMake(kButtonCoordinate[buttonIndex][0],kButtonCoordinate[buttonIndex][1],kButtonCoordinate[buttonIndex][2],kButtonCoordinate[buttonIndex][3]);
	
	tipMethodControlSwitch.segmentedControlStyle = UISegmentedControlStyleBar;
	tipMethodControlSwitch.tintColor = [UIColor colorWithRed:kSwitchColor[0]  green:kSwitchColor[1]  blue:kSwitchColor[2] alpha:kSwitchColor[3]];
	...
	tipMethodControlSwitch.selectedSegmentIndex = tipMethodIndex;
	[tipMethodControlSwitch addTarget:self action:@selector(tipMethodControlSwitchPressed:) forControlEvents:UIControlEventValueChanged];
}
...
- (void)refreshDisplay{
	...
	if(kRoundMode==0){
		...
		if(tipMethodIndex==0){
			...
			//NSLog(@"BA: %f, TP: %d, TT: %f, TE: %f, TEA: %f, TB: %f",billAmount,tipPercent,totalTip,tipEach,totalEachAmount, totalBillAmout);
		}
...
- (void)keyPadButtonPressed:(id)sender {
	...
	//NSLog(@"NBP: Bill Amount: %.3f (digits: %d, fractional digits: %d)\n",billAmount,numberOfDigits,numberOfFracDigits);
	// display new amount
	[self refreshDisplay];
}
...
- (void)tipMethodControlSwitchPressed:(id)sender {
	[self updateAppStateTo:1];
	tipMethodIndex = [sender selectedSegmentIndex];
	[self refreshDisplay];
}
